...
public class FileController {
	...
	public ResponseEntity<byte[]> delete(String path, @RequestParam(value = "name", required = false) String name,
							 Model model, HttpServletResponse response) throws Exception {
		...
		return delete(group, remoteFileName, specFileName);
	}

	private ResponseEntity<byte[]> delete(String groupName, String remoteFileName, String specFileName)
			throws Exception {
		int ret=fileApiService.delete(groupName, remoteFileName);
		HttpHeaders headers = new HttpHeaders();
		try {
			headers.setContentDispositionFormData("attachment",
					new String(specFileName.getBytes("UTF-8"), "iso-8859-1"));
		...
		return new ResponseEntity<byte[]>("delete Ok!".getBytes(), headers, HttpStatus.CREATED);
	}
}
